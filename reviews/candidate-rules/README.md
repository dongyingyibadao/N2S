...
No rule is approved, so all thresholds and `approved_scope` remain null and every manifest remains `candidate`
with `auto_apply: false`. The entries in `required_before_decision` are the evidence required before a future
